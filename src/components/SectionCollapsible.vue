<!--
  SectionCollapsible.vue - 可折叠卡片区域
  复用 ps-layer-tool 的设计和 CSS
-->
<template>
...
function loadExpanded(): boolean {
  try {
    const raw = localStorage.getItem("exportLayerTool.sectionStates.v1");
...
function toggle() {
  ...
  try {
    ...
    localStorage.setItem("exportLayerTool.sectionStates.v1", JSON.stringify(states));
  } catch { /* ignore */ }
}
</script>
